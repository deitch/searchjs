...
(function(exports){
	...
	singleMatch = function(field,s) {
		var oneMatch = false, t, re, i, j;
	    ...
	    oneMatch = false;
		  ...
		  for (j=0;j<field.length;j++) {
		    oneMatch = singleMatch(field[j],s);
        if (oneMatch) {
          break;
        }
		  }
	...
	matchObj = function(obj,search) {
		var i, j, matched, oneMatch, t, re, reMatch, negator, joinAnd, ary;
		search = search || {};
		...
		if (search.terms) {
          ...
          // match each one, if search[i] is an array - just concat to be safe
			    ...
			    for (j=0;j<ary.length;j++) {
			      oneMatch = (ary[j] !== undefined && obj[i] !== undefined) ? oneMatch = singleMatch(obj[i],ary[j]) : false;
            if (oneMatch) {
              break;
            }
          }
