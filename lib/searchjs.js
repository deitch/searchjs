...
(function(exports){
	var matchObj, _matchObj, _getOptions, _globalDefaults, matchArray, singleMatch, deepField, setGlobalDefaults;

	// Defines the default options which get chosen unless overwritten
	_globalDefaults = {
		negator: false,
		joinAnd: true,
		text: false,
	...
	setGlobalDefaults = function(options) {
		for (var key in options) {
			_globalDefaults[key] = options[key];
		}
	}
	...
	deepField = function(data,propertyPath,propertySearch,propertySearchDepth) {
		...
		if (data === null || data === undefined || propertyPath === null || propertyPath === undefined || !Array.isArray(propertyPath) || propertyPath.length < 1) {
			...
			for (i=0;i<data.length;i++) {
				...
				itemValue = deepField(data[i], copyPropertyPath, propertySearch, newPropertySearchDepth);
	...
	_getOptions = function(search) {
		...
		options.negator = search._not ? true : _globalDefaults.negator;
		// do we join via AND or OR
		options.joinAnd = search._join && search._join === "OR" ? false : _globalDefaults.joinAnd;

		// did we have text or word search?
		options.text = search._text ? true : _globalDefaults.text;
		options.word = search._word ? true : _globalDefaults.word;
		options.separator = search._separator || _globalDefaults.separator;

		options.propertySearch = search._propertySearch ? true : _globalDefaults.propertySearch;
		options.propertySearchDepth = search._propertySearchDepth ? search._propertySearchDepth : _globalDefaults.propertySearchDepth;
	...
	exports.setGlobalDefaults = setGlobalDefaults;
