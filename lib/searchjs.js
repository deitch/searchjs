...
(function(exports){
	...
	deepField = function(data,propertyPath,propertySearch) {
		...
		if (data === null || data === undefined || propertyPath === null || propertyPath === undefined || !Array.isArray(propertyPath) || propertyPath.length < 1) {
			...
			for (i=0;i<data.length;i++) {
				...
				itemValue = deepField(data[i], copyPropertyPath, propertySearch);
				...
				for (i=0;i<propertyNames.length;i++) {
					...
					if (Array.isArray(propertyData)) {
						...
						propertyData.forEach(function(propertyDataItem) {
							var foundValue = deepField(propertyDataItem, propertyPath, propertySearch);
							if (foundValue !== null) {
								ret.push(foundValue);
							}
						});
						...
						var foundValue = deepField(propertyData, propertyPath, propertySearch);
						if (foundValue !== null) {
							ret = foundValue;
						}
					}
	...
	_getOptions = function(search) {
		...
		options.propertySearch = search._propertySearch ? true : false;
			...
			for(i in search) {
				if (search.hasOwnProperty(i) && i.indexOf("_") !== 0) {
					...
					for (j=0;j<ary.length;j++) {
						oneMatch = singleMatch(deepField(obj,searchTermParts,options.propertySearch),ary[j],options.text,options.word);
						if (oneMatch) {
							break;
						}
					}
