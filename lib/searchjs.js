...
(function(exports){
  var matchObj, matchArray, singleMatch, deepField, getValueDot;
  ...
  matchObj = function(obj,search) {
    ...
    if (search.terms) {
      ...
      for(var searchTerm in search) {
        var searchTermParts = searchTerm.split(separator);
        ...
        if (search.hasOwnProperty(searchTerm) && searchTerm.indexOf("_") !== 0) {
          ...
          for (j=0;j<ary.length;j++) {
            oneMatch = oneMatch = singleMatch(deepField(obj,searchTermParts),ary[j],text,word);
            if (oneMatch) {
              break;
            }
          }
