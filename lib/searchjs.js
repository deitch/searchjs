...
(function (global, factory) {
  ...
  function setDefaults(options) {
  	for (var key in options) {
  		exports._defaults[key] = options[key];
  	}
  }
  ...
  function _getOptions(search) {
  	...
  	options.negator = _getSingleOpt(search._not, exports._defaults.negator, false);
  	// do we join via AND or OR
  	//options.joinAnd = search._join && search._join === "OR" ? false : _defaults.join || true;
  	options.joinAnd = _getSingleOpt(search._join, exports._defaults.join, "AND") !== "OR";

  	// did we have text, word, start or end search?
  	options.text = _getSingleOpt(search._text, exports._defaults.text, false);
  	options.word = _getSingleOpt(search._word, exports._defaults.word, false);
  	options.start = _getSingleOpt(search._start, exports._defaults.start, false);
  	options.end = _getSingleOpt(search._end, exports._defaults.end, false);

  	options.separator = search._separator || exports._defaults.separator || '.';
  	options.propertySearch = _getSingleOpt(search._propertySearch, exports._defaults.propertySearch, false);
  	options.propertySearchDepth = _getSingleOpt(search._propertySearchDepth, exports._defaults.propertySearchDepth, -1);
